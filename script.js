...
function transitionToRoom() {
  // Sequence (total ~1100ms):
  //   Step 1 — exit fullscreen immediately so the image returns to normal DOM flow
  //   Step 2 — once fullscreenchange fires, animate the image shrinking (500ms)
  //   Step 3 — once shrink is done, slide the game card up into view (600ms)
  //   Step 4 — startRoom()
  //
  // Animating BEFORE exitFullscreen would be invisible because the fullscreen
  // rendering layer ignores CSS transforms in all major browsers.
  ...
  if (document.fullscreenElement) {
    ...
    document.exitFullscreen().catch(() => runShrink());
  } else {
    runShrink();
  }

  function runShrink() {
    // Image is now back in the document. Use double-rAF so the browser has
    // painted the restored layout before we start animating.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        roomImage.style.transition = 'transform 500ms cubic-bezier(0.4, 0, 0.2, 1), opacity 500ms cubic-bezier(0.4, 0, 0.2, 1)';
        roomImage.style.transformOrigin = 'center center';
        roomImage.style.transform = 'scale(0.85)';
        roomImage.style.opacity = '0';

        setTimeout(() => {
          // Reset image so it looks clean inside the card for the next render
          roomImage.style.transition = 'opacity 300ms ease';
  ...
  function runSlideIn() {
    const gameCard = document.querySelector('.game-card') || gameScreen;

    // Start the card from below, invisible
    ...
    // Spring-curve slide up into final position
    gameCard.style.transition = 'transform 600ms cubic-bezier(0.22, 1, 0.36, 1), opacity 500ms ease';
